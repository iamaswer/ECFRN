...
class FMRM(nn.Module):
    ...
    def __init__(self, hidden_size, inner_size=None, num_patch=25, drop_prob=0., reconstruction_attention_dropout=0.1):
        ...
        self.cfrm = CFRM(hidden_size=self.hidden_size, inner_size=self.hidden_size, drop_prob=0.1)
    ...
    def compute_distances(self, query_a, key_a, value_a, query_b, key_b, value_b, features_a, features_b):
        ...
        # (N_query x N_way x 1 x HW x N_shot*HW) x (1 x N_way x 1 x N-shot*HW x C) -> (N_query x N_way x 1 x HW x C)
        reconstructed_features_b = torch.matmul(att_probs, value_a)
        '''
        reconstructed_features_b <class 'torch.Tensor'> torch.Size([450, 30, 1, 25, 64])
        '''

        # Reconstructed features A,得到重构后的5张支持集
    ...
    def forward(self,features_a, features_b,phi):
        re_a1 = self.cfrm(features_a, features_a, phi)
        re_a1 = re_a1.view(features_a.size(0), re_a1.size(0) // features_a.size(0),  self.hidden_size, 5, 5).permute(0, 2, 1, 3, 4).contiguous()
        features_a = torch.cat((features_a, re_a1), axis=2)
